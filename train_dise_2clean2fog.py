...
# Data-related
LOG_DIR = './log/2clean2fz_medium_new_var/s2t1/'
GEN_IMG_DIR = './generated_imgs/2clean2fz_medium_new_var/s2t1/'
...
#DATA_LIST_PATH_CITY_IMG = './util/loader/cityscapes_list/train_syn.txt'
#DATA_LIST_PATH_CITY_LBL = './util/loader/cityscapes_list/train_syn_label.txt'

# source image list
DATA_LIST_PATH_CITY_IMG = './util/loader/cityscapes_list/city_fz_clean.txt'
DATA_LIST_PATH_CITY_LBL = './util/loader/cityscapes_list/city_fz_clean.txt'
'''  
...
DATA_LIST_PATH_CITY_FOG_IMG = './util/loader/cityscapes_list/train_fz_medium+test.txt'
#DATA_LIST_PATH_CITY_FOG_IMG = './util/loader/cityscapes_list/train_fz_clean.txt'
...
target_set = ZurichLoader(args.target_data_path, args.data_list_path_gta5, max_iters=num_steps * batch_size,
                        crop_size=source_input_size, transform=city_data_aug, mean=IMG_MEAN, set='fz_medium')
target_loader = torch_data.DataLoader(target_set, batch_size=batch_size, shuffle=True, num_workers=4, pin_memory=True)

city_set = ZurichTrainLoader(args.city_data_path, args.data_list_path_city_img, args.data_list_path_city_lbl,
...
#load_models(model_dict, './results/fz_clean/weight_best')
load_models(model_dict, load_model_path)


# enc_shared.load_state_dict(torch.load('./results/city2fz_clean_new/weight_best/enc_shared.pth'))    # already updated enc_shared network
# model_dict['enc_shared'] = enc_shared
...
code_s_previous = torch.rand(1,8)
...
for i_iter in range(num_steps):
    ...
    code_s_private = enc_s(low_s)   # invariant with fog
    ...
    loss_s_invariant = mse_loss(code_s_private, code_s_previous.cuda())
    ...
    code_s_previous = code_s_private
    ...
    if i_iter % 25 == 0:
        ...
        plt.title('s_code_invariant_loss')
        loss_s_invariant_tmp.append(loss_s_invariant.item())
        plt.plot(i_iter_tmp, loss_s_invariant_tmp, label='loss_s_code')
        plt.legend(bbox_to_anchor=(0., 1.02, 1., .102), loc=3, borderaxespad=0.)
        plt.grid()
        plt.savefig(os.path.join(args.log_dir, 's_code_loss.png'))
        plt.close()
    ...
    if i_iter % 15 == 0 or i_iter % 15 == 1 : # save imgs
