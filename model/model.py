...
class SharedEncoder(nn.Module):
    ...
    def forward(self, x):
        ...
        rec= self.layer4(x) 
        # x2 = self.final2(rec)
        x2 = self.final22(torch.cat((rec, x), 1))
...
class SharedEncoder_var(nn.Module):
    def __init__(self):
        super(SharedEncoder_var, self).__init__()
        self.n_classes = pspnet_specs['n_classes']
...
class DomainClassifier(nn.Module):   # FCN classification layer
    def __init__(self):
